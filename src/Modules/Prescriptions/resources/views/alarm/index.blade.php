...
<x-breadcrumbs :breadcrumbs="$breadcrumbs" />
    <div class="flex flex-col lg:flex-row gap-6 p-8" style="background-color: #EFF2FB;" ng-app="myApp" ng-controller="AlarmController as main">
        <loading></loading>
        <!-- Left Column -->
        <div class="w-full lg:w-1/3 flex flex-col gap-4">
            <div class="relative bg-white rounded-2xl shadow-xl overflow-hidden mb-6">
                ...
                <img src="{{ asset('resources/bg_abstract_3.png') }}" alt="CuraTempus" class="w-full h-64 object-cover rounded-t-2xl z-0">
                <div class="absolute bottom-6 left-6 text-4xl font-bold z-20">
                    <span class="text-white">Cura</span><span class="text-gray-300">Tempus</span>
                </div>
            </div>
                    ...
                    <table class="min-w-full text-sm text-left">
                        ...
                        <tbody>
                            <tr ng-repeat="row in main.gridData" class="border-b last:border-0">
                                ...
                                <td class="py-2 px-3">
                                    <span ng-if="row.status != 2" ng-class="{'text-green-600': row.status == 1, 'text-red-600': row.status == 0}">
                                        @{{ row.status == 1 ? 'Success' : 'Failed' }}
                                    </span>
                                    <a ng-if="row.status == 2" href="/alarm/configure/@{{ row.id }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-1 rounded">
                                        Pending
                                    </a>
                                </td>
                                <td class="py-2 px-3">
                                    <a href="/alarm/view/@{{ row.id }}" class="bg-blue-500 hover:bg-blue-700 text-white px-4 py-1 rounded">View</a>
                                </td>
